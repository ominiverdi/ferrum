...
The `bash` and `wait` tools parse complete Bash syntax and apply a tiered execution policy. **Low** grants broad current-user shell authority and bypasses writable roots. The default **medium** tier supports direct trusted-checkout development, rejects dynamic/indirect authority and shell/interpreter payloads, and limits native and recognized shell mutations to `[tools].writable_roots`. **High** is inspection-only and rejects native/shell mutation, networking, interpreters, builds, and unknown executables. Tier-independent resource, cancellation, catastrophic-operation, privilege, protected-credential, target-identity, and atomic-replacement checks remain. Native `read`, `grep`, `find`, and `ls` enforce bounded line/result/traversal behavior, with cancellation and deadlines for searches. Use **--safety low|medium|high** at startup or **/safety low|medium|high** interactively. This policy is not a sandbox.
