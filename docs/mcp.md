...
`/mcp status` and `/mcp list` show configured servers after any `--mcp <server...>` narrowing. Tool counts distinguish available definitions from the tools actually exposed after `--tools`, `--no-tools`, and `[tools]` policy; `total_tools_exposed` and `tool_schema_bytes` match that resolved model-facing set. Before configured MCP servers are first discovered, exposure and schema fields are reported as `undiscovered` instead of validating policy against an incomplete definition set.
...
MCP stderr content is not included in tool errors or model context. Errors expose only bounded summaries indicating that diagnostics were withheld. MCP child processes receive a small baseline of non-secret process/session variables plus variables explicitly named in their per-server allowlist. On Linux sessions with delegated cgroup-v2 children, each MCP server tree runs in a private child cgroup and `cgroup.kill` bounds descendants when the server is dropped. If cgroup setup is unavailable, Ferrum logs the reason and falls back to process-group cleanup; descendants that deliberately leave that group may survive.
