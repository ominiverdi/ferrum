...
```bash
scripts/sync-release-version.sh v0.7.7
...
```bash
FERRUM_REPRODUCIBILITY_CHECK=1 scripts/package-linux.sh v0.7.7
```
...
```text
ferrum-v0.7.7-x86_64-unknown-linux-gnu.tar.gz
ferrum-v0.7.7-x86_64-unknown-linux-gnu.tar.gz.sha256
ferrum_0.7.7_amd64.deb
ferrum_0.7.7_amd64.deb.sha256
ferrum-0.7.7-1.x86_64.rpm
ferrum-0.7.7-1.x86_64.rpm.sha256
```
...
sha256sum -c ferrum_0.7.7_amd64.deb.sha256
sha256sum -c ferrum-0.7.7-1.x86_64.rpm.sha256
dpkg-deb --info ferrum_0.7.7_amd64.deb
dpkg-deb --contents ferrum_0.7.7_amd64.deb | head
```
...
```bash
curl -LO https://codeberg.org/ominiverdi/ferrum/releases/download/v0.7.7/ferrum_0.7.7_amd64.deb
...
```bash
curl -LO https://codeberg.org/ominiverdi/ferrum/releases/download/v0.7.7/ferrum-0.7.7-1.x86_64.rpm
sudo dnf install ./ferrum-0.7.7-1.x86_64.rpm
ferrum --help
```

Use `sudo zypper install ./ferrum-0.7.7-1.x86_64.rpm` on openSUSE.
