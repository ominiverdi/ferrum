...
```bash
cargo build --release
scripts/package-linux.sh v0.6.1
```

The script writes assets to `dist/`:

```text
ferrum-v0.6.1-x86_64-unknown-linux-gnu.tar.gz
ferrum-v0.6.1-x86_64-unknown-linux-gnu.tar.gz.sha256
ferrum_0.6.1_amd64.deb
ferrum_0.6.1_amd64.deb.sha256
ferrum-0.6.1-1.x86_64.rpm
ferrum-0.6.1-1.x86_64.rpm.sha256
```
...
sha256sum -c ferrum_0.6.1_amd64.deb.sha256
sha256sum -c ferrum-0.6.1-1.x86_64.rpm.sha256
dpkg-deb --info ferrum_0.6.1_amd64.deb
dpkg-deb --contents ferrum_0.6.1_amd64.deb | head
```
...
```bash
curl -LO https://codeberg.org/ominiverdi/ferrum/releases/download/v0.6.1/ferrum_0.6.1_amd64.deb
sudo apt install ./ferrum_0.6.1_amd64.deb
ferrum --help
```
...
```bash
curl -LO https://codeberg.org/ominiverdi/ferrum/releases/download/v0.6.1/ferrum-0.6.1-1.x86_64.rpm
sudo dnf install ./ferrum-0.6.1-1.x86_64.rpm
ferrum --help
```

Use `sudo zypper install ./ferrum-0.6.1-1.x86_64.rpm` on openSUSE.
